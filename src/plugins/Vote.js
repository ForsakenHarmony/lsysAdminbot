...
module.exports = class Vote {
  ...
  async prepare() {
    ...
    let kbd = TGExtra.markdown().markup(m =>
      m
    ...
    this.votemsg = await this.tg.sendMessage(
      this.chatId,
      `I am putting this to a vote! ${this.question}`,
      kbd
    );
    this.resultmsg = await this.tg.sendMessage(
      this.chatId,
      'There have been no votes, yet.'
    );
  ...
  async evaluate() {
    ...
    let { done, msg } = await this.cb(
      { sum, results: res, votes: this.votes },
      ...this.cbargs
    );
    this.done = done;
    ...
    let resTxt = '';
    if (this.anonymized) {
      resTxt = Object.keys(res)
        .map(
          k =>
            `${res[k]} vote${res[k] != 1 ? 's' : ''} ${
              res[k] != 1 ? 'say' : 'says'
            } '${k}'`
        )
        .join('\n');
    } else {
      resTxt = Object.keys(this.votes)
        .map(k => `[${k}](tg://user?id=${k}) said '${this.votes[k]}'`)
        .join('\n');
    }
    let txt = `There ${sum != 1 ? 'have' : 'has'} been ${sum} vote${
      sum != 1 ? 's' : ''
    } so far${sum != 0 ? ':' : '.'}\n`;
    txt += resTxt;
    txt += msg ? '\n' + msg : '';
