...
class KickVote extends Vote {
  ...
  async processVotes(res) {
    let done = false;
    let msg = `This vote expires in ~${Math.round(
      (this.expiredAt - new Date()) / 1000 / 60
    )} minutes.`;
    msg += `\n${Math.round(
      (res.results['Yes'] * 100) / res.sum
    )}% voted in favour of this (${KICK_REQUIRE * 100}% required).`;
    ...
    if (this.expiredAt < new Date()) {
      ...
      if (res.results['Yes'] / res.sum > KICK_REQUIRE) {
        let extras = {
          until_date: Math.round(new Date() / 1000 + KICK_TIMEOUT * 60),
        };
        await this.tg
          .kickChatMember(this.chat.id, this.user.id, extras)
          .then(() => {
...
module.exports = class Kick {
  ...
  help(ctx) {
    let help = [`/kick @username - start a vote to kick @username.`];
    return help.join('\n');
  }
  settings(ctx) {
    let set = [
      `Require more than ${KICK_REQUIRE *
        100}% of all votes within ${KICK_VOTE_DURATION} minutes to kick a user.`,
